...
"""index.html 을 헤드리스 크롬에 띄워 동작을 확인한다.
   ...
   🔴 2026-09-11 ④ 실데이터가 들어온 뒤에도 빈 지역을 샘플로 채우는 바람에
         가 본 적 없는 곳의 핀·예시 배지가 진짜 기록에 섞여 보이던 것
"""
...
# 실데이터가 한 곳이라도 있으면 샘플은 단 한 건도 섞이면 안 된다.
ONLY_BIEI = [
...
setTimeout(async () => {
  ...
  out.samplePlaces = PLACES.filter(p => p._sample).length;
  out.exSpots = ex.querySelectorAll('.spot').length;
  out.errors = window.__err;
  const d = document.createElement('pre');
  d.id = 'PROBE'; d.textContent = JSON.stringify(out);
...
print("④ 실데이터가 비에이 한 곳뿐일 때 — 샘플 장소가 단 하나도 섞이지 않는가")
r = run(ONLY_BIEI, POSTER_PROBE)
ck("자바스크립트 오류 없음", r["errors"], [])
ck("실데이터 모드다", r["isLive"], True)
ck("샘플 장소가 하나도 없다", r["samplePlaces"], 0)
ck("장소는 실데이터 한 곳뿐", r["places"], 1)
ck("비에이만 핀이 선다", r["byG"], {"biei": 1})
ck("비에이는 실데이터 한 곳", r["poster"]["biei"]["cards"], 1)
ck("삿포로 패널은 카드가 없다", r["poster"]["sapporo"]["cards"], 0)
ck("오타루 패널은 카드가 없다", r["poster"]["otaru"]["cards"], 0)
ck("삿포로 패널은 「사진 없음」으로 둔다", r["poster"]["sapporo"]["nospot"], True)
ck("오타루 패널은 「사진 없음」으로 둔다", r["poster"]["otaru"]["nospot"], True)
ck("패널 머리글에 예시 표시가 없다", any(v["sample"] for v in r["poster"].values()), False)
ck("펼쳐보기에 예시 안내가 없다", r["exBanner"], False)
ck("펼쳐보기에 예시 배지가 없다", r["exSampleBadges"], 0)
ck("펼쳐보기도 실데이터 한 곳", r["exSpots"], 1)
