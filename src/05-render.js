...
/* ══ 사진 ══
   ...
   🔴 사진이 보이려면 드라이브 파일이 「링크가 있는 사람」에게 열려 있어야 한다.
      닫혀 있으면 어떤 주소를 써도 구글 로그인 화면으로 넘어가 전부 깨진다
      (폰 카카오 브라우저처럼 구글에 로그인 안 된 곳에서 특히).
      여는 일은 Apps Script 가 한다 — sapporo-trip-settlement/apps-script/Share.js,
      새 사진은 갱신할 때 자동, 이미 쌓인 사진은 ?action=sharePhotos.

   주소는 lh3(구글 사진 CDN) 를 먼저 쓴다. drive.google.com/thumbnail 은 한 장마다
   다른 주소로 한 번 더 튕기고 여러 장을 한꺼번에 부르면 막히는 일이 있다 —
   펼쳐보기처럼 수십 장을 한 화면에 까는 곳에서 불리하다.
   그래도 못 받아 오면 바로 아래 error 처리기가 옛 주소로 한 번 더 시도한다. */
const photoUrl = (id, w) => `https://lh3.googleusercontent.com/d/${encodeURIComponent(id)}=w${w}`;
const photoUrlAlt = (id, w) => `https://drive.google.com/thumbnail?id=${encodeURIComponent(id)}&sz=w${w}`;

/* lh3 가 실패한 사진만 옛 주소로 한 번 더 — 사진마다 손댈 필요 없이 여기서 받는다.
   error 는 위로 올라오지 않으므로(버블링이 없다) 캡처 단계에서 잡는다. */
addEventListener('error', e => {
  const n = e.target;
  if (!n || n.dataset == null || n.dataset.fb) return;
  const cur = n.tagName === 'IMG' ? n.src
            : (n.tagName === 'image' ? n.getAttribute('href') : '');
  const m = /lh3\.googleusercontent\.com\/d\/([^=?]+)=w(\d+)/.exec(cur || '');
  if (!m) return;
  n.dataset.fb = '1';
  const u = photoUrlAlt(decodeURIComponent(m[1]), m[2]);
...
(function charDrag(){
  ...
  document.addEventListener('pointerdown', e => {
    ...
    /* 🔴 지도 탭만 보고 있었다 — 인쇄 탭은 .mapwrap 이 없어 꾹 누르기가 아예 안 걸렸다.
          인쇄 탭 지도는 .p-panel[data-region] > .p-map 안의 pover-* SVG 다. (2026-09-11) */
    const panel = e.target.closest('.p-panel[data-region]');
    if (panel){ if (!e.target.closest('.p-map')) return; }   // 패널 머리글(캡션)은 지도가 아니다
    else if (!e.target.closest('.mapwrap')) return;
...
/* 이미 서 있는 캐릭터 자리 — 그려진 것에서 그대로 읽는다 (지도 레이어 · 인쇄 패널 공용) */
function charTakenIn(root){
  return [...root.querySelectorAll('image.charmove')].map(im => ({
    x: parseFloat(im.getAttribute('x')), y: parseFloat(im.getAttribute('y')),
    w: parseFloat(im.dataset.w),         h: parseFloat(im.dataset.h) }));
}

/* 인쇄 패널의 장애물 — 사진 카드·핀·이름표를 그려진 그대로 읽는다.
   카드와 글자는 넓어서 가운데 한 점으로는 모자라 격자로 흩어 놓는다. */
function charObsPrint(over, box){
  const obs = [];
  const step = Math.min(box[2], box[3]) * 0.03;
  over.querySelectorAll('image[data-pi], rect[width], text, circle').forEach(el => {
    if (el.classList.contains('charmove') || el.classList.contains('pbg')) return;
    let b;
    try { b = el.getBBox(); } catch (_) { return; }
    if (!b || !isFinite(b.x)) return;
    if (b.width < step && b.height < step){ obs.push([b.x + b.width/2, b.y + b.height/2]); return; }
    for (let x = b.x; x <= b.x + b.width; x += step)
      for (let y = b.y; y <= b.y + b.height; y += step) obs.push([x, y]);
  });
  return obs;
}

/* 꾹 눌러 캐릭터를 더할 때 재는 상자 · 눈높이 · 장애물 — 탭마다 좌표계가 다르다.
   지도 탭은 mapillust 뷰박스 전체, 인쇄 탭은 패널마다 따로 잡은 지도 상자(cbox)다. */
function charPutCtx(where, region){
  if (where === 'print'){
    const over = document.querySelector(`.p-panel[data-region="${region}"] svg[id^="pover-"]`);
    const cb = over && over.dataset.cbox;
    if (!cb) return null;                          // 아직 한 번도 그리지 않은 패널
    const box = cb.split(' ').map(Number);
    if (box.length !== 4 || box.some(n => !isFinite(n)) || !box[2] || !box[3]) return null;
    return { box: box,
             propH: box[2] * 0.245 * 0.89,         // drawPanel 의 sizes.prop 과 같은 눈높이
             taken: charTakenIn(over),
             obs:   charObsPrint(over, box) };
  }
  const v = mapillust.viewBox.baseVal;
  return { box: [0, 0, v.width, v.height], propH: v.width * 0.125,
           taken: charTakenIn(mapillust), obs: charObsNow() };
}
...
function charPutNear(where, region, sprite, at){
  ...
  if (!s) return;
  const ctx = charPutCtx(where, region);
  if (!ctx) return;
  const box = ctx.box;
  const h = ctx.propH, w = h * s.w / s.h;         // 소품과 같은 눈높이
  /* 자리 고르기는 세 걸음이다.
  ...
  if (!best){
    const cands = freeSpots(box, w, h, ctx.obs, 999, taken);
    best = cands[0];
    if (at && cands.length){
...
function pinSvg(x, y, color, i, on){
  const r = 8.0;                        // 사진 장수와 무관하게 늘 같은 크기
  return `<g class="pin${on?' on':''}" data-i="${i}" tabindex="0" role="button">`
    // 손가락이 닿을 자리만 넓게 잡는다 — 칠하지 않으므로 눈에는 보이지 않는다.
    // 🔴 예전에는 이 원을 핀 색으로 꽉 채워서(r*2.4) 핀이 물방울이 아니라
    //    커다란 동그라미로 보였다. 폰에서 특히 심했다. (2026-09-11)
    + `<circle class="hit" cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${(r*2.4).toFixed(1)}" fill="transparent" pointer-events="all"/>`
...
/* 핀이 너무 가까우면 퍼뜨린다 — 손가락 터치 반경(r*2.4≈19px) 안에 다른 핀이 있으면 밀어낸다.
   SVG 좌표계 기준이라 실제 화면 px 와는 vb 배율만큼 다를 수 있지만, 같은 공간에서 반복하므로 일관성 있다.
   최대 3회 반복으로 자연스럽게 수렴한다. */
function spreadPins(coords, minDist){
  ...
  for (let iter = 0; iter < 4; iter++){
    ...
    for (let a = 0; a < pts.length; a++){
      for (let b = a+1; b < pts.length; b++){
        ...
        pts[a].x -= nx * push; pts[a].y -= ny * push;
        pts[b].x += nx * push; pts[b].y += ny * push;
        moved = true;
      }
    }
    if (!moved) break;
...
function drawScreen(){
  ...
  let pins = '';
  // 핀 좌표 수집
  const rawCoords = vs.map(({p}) => R.px(p.lat, p.lon));
  // 겹치는 핀 퍼뜨리기 — 터치 반경(r*2.4=19.2) 기준으로 밀어낸다
  const spreadCoords = spreadPins(rawCoords, 20);
  vs.forEach(({p,i}, idx) => {
    const [x,y] = spreadCoords[idx];
    pins += pinSvg(x, y, `var(${CAT[p.k].c})`, i, i===openIdx);
  });
...
function drawPanel(R, side, baseEl, overEl, maxCard){
  ...
  // 카드는 사진 많은 곳 우선 — ph(서버 집계)와 shots.length(로컬 로드) 중 큰 값 사용
  const rank = [...spots].sort((a,b)=>{
    const pa = Math.max(a.ph||0, (PLACES[a.src]&&PLACES[a.src].shots?PLACES[a.src].shots.length:0));
    const pb = Math.max(b.ph||0, (PLACES[b.src]&&PLACES[b.src].shots?PLACES[b.src].shots.length:0));
    return pb - pa;
  }).slice(0, maxCard);
  ...
  // 꾹 눌러 캐릭터를 더할 때 이 상자를 다시 읽는다 (charPutCtx)
  overEl.dataset.cbox = [bx, by, bw, bh].map(n => n.toFixed(1)).join(' ');
...
function buildExpand(){
  let html = '';
  // 드라이브에 사진이 하나도 없어 통째로 샘플일 때만 밝힌다 —
  // 실데이터가 있으면 샘플은 한 건도 섞이지 않으므로 이 띠는 뜨지 않는다.
  ...
  DAYS.forEach((d, di) => {
    ...
    list.forEach(({p,i}) => {
      ...
      for (let k=0;k<p.ph;k++)
        ph += `<div class="ph${k===p.hero?' hero':''}" data-pi="${i}" data-k="${k}" style="cursor:zoom-in">${shotImg(p,k,i*7+k,320)}<button class="star" aria-pressed="${k===p.hero}" aria-label="대표 사진">★</button></div>`;
  ...
  // 펼쳐보기 사진 클릭 → 라이트박스 / ★ 클릭 → 대표 사진 설정 (한 번만 등록)
  const exbody = document.getElementById('exbody');
  if (!exbody._lbBound) {
    exbody._lbBound = true;
    exbody.addEventListener('click', function(e){
      // ★ 버튼 클릭 — 대표 사진 설정
      const star = e.target.closest('.star');
      if (star){
        e.stopPropagation();
        const ph = star.closest('[data-pi]');
        if (!ph) return;
        const pi = +ph.dataset.pi, k = +ph.dataset.k;
        const p = PLACES[pi]; if (!p) return;
        // 같은 장소의 ★ 전부 해제 후 이 장 선택
        const article = ph.closest('article');
        article.querySelectorAll('.star').forEach(s => s.setAttribute('aria-pressed','false'));
        article.querySelectorAll('.ph').forEach(d => d.classList.remove('hero'));
        star.setAttribute('aria-pressed','true');
        ph.classList.add('hero');
        p.hero = k;
        saveHero(p);
        posterRefresh();
        return;
      ...
      // 사진 썸네일 클릭 → 라이트박스
      const ph = e.target.closest('[data-pi]');
      if (!ph) return;
      const pi = +ph.dataset.pi, k = +ph.dataset.k;
      openPlacePhotos(pi, k);
    });
  }
}
