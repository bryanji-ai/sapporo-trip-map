...
const CHAR_NAMES = {
  ...
  'cp-c-passport':'여권커플', 'cp-c-kaminari':'아사쿠사', 'cp-c-icebar':'아이스바',
  'cp-c-momiji':'단풍커플2', 'cp-c-fuji':'후지산', 'cp-c-yukata':'유카타',
  'cp-c-neko':'마네키네코', 'cp-c-softcream':'소프트크림', 'cp-c-tower':'도쿄타워',
  'cp-c-onsen':'온천2', 'cp-c-sunflower':'해바라기', 'cp-c-harbor':'항구뷰',
  'cp-c-tako':'타코야키', 'cp-c-snowman':'눈사람3', 'cp-c-sushi':'스시',
  'cp-c-otaru':'오타루',
...
const CHAR_PICK_ALL = [
  ...
  'cp-c-passport','cp-c-kaminari','cp-c-icebar','cp-c-momiji',
  'cp-c-fuji','cp-c-yukata','cp-c-neko','cp-c-softcream',
  'cp-c-tower','cp-c-onsen','cp-c-sunflower','cp-c-harbor',
  'cp-c-tako','cp-c-snowman','cp-c-sushi','cp-c-otaru',
...
/* 구성(추가·교체·삭제)도 자리와 마찬가지로 서버에 올린다.
   한쪽이 캐릭터를 지웠는데 다른 쪽에 그대로 남으면 헷갈리기 때문이다.
   지역 단위로 통째로 보낸다 — 항목이 몇 개뿐이라 GET 으로 충분하다. */
function pushCharSet(where, region){
  charStoreSave();
  if (typeof WEB_APP === 'undefined') return;
  const k = where + ':' + region;
  const st = CHARSET[k];
  const empty = !st || (!st.del.length && !st.add.length && !Object.keys(st.swap).length);
  const u = `${WEB_APP}?action=setCharSet&k=${encodeURIComponent(k)}`
          + (empty ? '&v=' : `&v=${encodeURIComponent(JSON.stringify(st))}`);
